...
The dataset contains roughly 3600 documents.  Additional metadata elements can be accessed via the [Federal Register API](https://www.federalregister.gov/developers/documentation/api/v1#/).

A CSV of some of these metadata for documents can be found in ./data

## Challenge

As regulatory priorities of federal agencies evolve over time, new regulatory issues and topics will emerge.  **A useful output of this challenge should include a method, model, or tool to help identify these emerging issues as they occur** and classify them in future Federal Reserve publications.  Some starting questions might be: 
...
This is an open ended challenge!  Feel free to tackle as many or as few of the above prompts.  Solutions can range from end to end solutions that label documents as they are published to a process to flag interesting new documents for editors to examine.
...
4.  Some events for testing your methods and approaches include the housing crisis of 2008 (and prior/subsequet rulemaking leading up to it) and the ongoing COVID-19 crisis.  A good unsupervised approach would identify both events as new clusters.
